...
const allHeaders = ref([
    ...
    { title: 'N宝+幸运6+对子洗码总分', value: 'xml_sb', minWidth: 200, copyable: true },
    { title: '庄闲赢亏总分', value: 'zx_yl', minWidth: 120, copyable: true },
    { title: 'N宝+幸运6+对子总分', value: 'sb_yl', minWidth: 200, copyable: true },
    ...
    try {
        ...
        if (res.code == 200) {
            const data = res.data.list.map(item => ({
                ...
                '庄闲洗码总分': item.xml_zx,
                'N宝+幸运6+对子洗码总分': item.xml_sb,
                '庄闲赢亏总分': item.zx_yl,
                'N宝+幸运6+对子总分': item.sb_yl,
                '有效流水总分': item.yxxz,
